...
@section('content')
<div class="container">
  <h1 class="mb-4">Editar Item #{{$item->id}}</h1>
  <form method="POST" action="{{route('expense-items.update',$item)}}" class="card p-4 shadow-sm">
    @csrf
    @method('PUT')
    @if($errors->any())
    ...
    <!-- Se elimina lógica de tipo y monto: sólo nombre y estado activo -->
